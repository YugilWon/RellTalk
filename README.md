...
이 프로젝트는 이메일 인증 이후에만 사용자 프로필이 생성되도록 설계했습니다.
...
회원가입 직후 Supabase가 자동으로 로그인 세션을 발급하는 문제를 확인.  
이를 해결하기 위해 회원가입 직후 세션을 제거하고, 인증 완료 후 콜백 단계에서 세션을 재확인한 뒤 profiles를 생성하도록 처리했습니다.

이를 통해 인증되지 않은 사용자의 데이터 생성을 방지했습니다.
